...
public class Device implements IApp, IPinAuthObserver {
     ...
     * @return return the orientation state: portrait or landscape
     */
    ...
    /**
     * set pin start the device and return New Instance 
     * @param pin the pin 
    ...
    public void startUp()
    {
        ...
        kp = new KeyPad() ;
        if(fourPin) {
        	pc = new Passcode(true) ;
        }
        if(sixPin) {
        	pc = new Passcode(false) ;
        }
        sp = new Spacer() ;
        ps = new PinScreen() ;
        pm = new PinEntryMachine(pin) ;

        // setup the composite pattern
        ps.addSubComponent( pc ) ;
        ps.addSubComponent( sp ) ;
        ps.addSubComponent( kp ) ;
        //System.err.println("ps "+ps.display());
    ...
    */
    ...
    /**
     * handle the situation of pin validation failure
     */
    public void authEventFalse() {
